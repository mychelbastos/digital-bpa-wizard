-- Tabulação e Relação por procedimento passam a aceitar VÁRIOS procedimentos (_procs text[]).
-- NULL/vazio = todos. Substitui as versões de parâmetro único (_proc text).
drop function if exists public.tabulacao_producao(text[], text, text, text, text);
drop function if exists public.relacao_pacientes_producao(text[], text, text, text, text);

-- ===== Relação nominal por produção (RAAS / por procedimento[s]) =====
...
-- ===== Tabulação por procedimento[s] (agregada, por atendimento/quantidade) =====
