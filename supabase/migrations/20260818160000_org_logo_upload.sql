-- Logo/timbre dos relatórios agora é editável pelo formulário do admin (upload → base64).
-- Estende os RPCs de organização para ler e gravar organizacoes.logo_relatorio.

-- admin_organizacoes: inclui logo_relatorio no retorno (muda a assinatura → recria).
...
returns table(id uuid, nome text, municipio_ibge text, uf text, cab_orgao_origem text,
  ...
  cor_relatorio text, logo_relatorio text, gestao_id uuid, gestao_nome text, gestao_inicio date, gestao_fim date)
language sql stable security definer set search_path to 'public' as $$
  select o.id, o.nome, o.municipio_ibge, o.uf,
    ...
    o.cor_relatorio, o.logo_relatorio,
...
-- admin_salvar_organizacao: novo parâmetro _logo (data URI base64 PNG).
--   _logo null       → mantém a logo atual (não mexe)
--   _logo ''         → remove a logo
--   _logo 'data:...' → grava a nova logo
drop function if exists public.admin_salvar_organizacao(uuid, text, text, text, text, text, text, text, text, text, text);
create function public.admin_salvar_organizacao(_org uuid, _nome text, _ibge text, _uf text,
  _orig text, _sigla text, _cgc text, _dest text, _dtipo text, _versao text, _cor text default null, _logo text default null)
...
begin
  ...
  update public.organizacoes set
    ...
    logo_relatorio = case when _logo is null then logo_relatorio
                          when btrim(_logo) = '' then null
                          else _logo end
  where id = _org;
end $$;
grant execute on function public.admin_salvar_organizacao(uuid, text, text, text, text, text, text, text, text, text, text, text) to authenticated;
