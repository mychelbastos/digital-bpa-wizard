...
import { sincronizarConfigDaOrg, configCompleta } from "@/lib/bpa-i-v2/config";
  ...
  if (s.tipo === "ok")
    return (
      <span className="inline-flex items-center gap-1 text-xs font-medium text-emerald-600">
        <Check className="size-3.5" /> {s.msg ?? "Salvo"}
      </span>
    );
  if (s.tipo === "erro")
    return (
      <span className="inline-flex items-center gap-1 text-xs font-medium text-destructive">
        <AlertCircle className="size-3.5" /> {s.msg ?? "Erro"}
      </span>
    );
  return null;
}
...
function Perfil() {
  ...
  // A config do cabeçalho passou a ser da ORGANIZAÇÃO. Se a org tiver config, mostramos
  // (somente leitura) e apontamos para a Administração.
  const [configDaOrg, setConfigDaOrg] = useState(false);
  ...
  useEffect(() => {
    ...
    // Se a organização já tem o cabeçalho configurado, ele é a fonte da verdade.
    sincronizarConfigDaOrg().then((cfg) => {
      if (cfg && configCompleta(cfg)) {
        setConfigDaOrg(true);
        setPerfil((p) => ({ ...p, config: cfg }));
      }
    });
  }, []);

  const set = <K extends keyof PerfilDados>(k: K, v: PerfilDados[K]) =>
    setPerfil((p) => ({ ...p, [k]: v }));
  ...
  const salvarSenha = async () => {
    ...
    if (novaSenha !== confirmaSenha)
      return setStatusSenha({ tipo: "erro", msg: "As senhas não conferem" });
    setStatusSenha({ tipo: "loading" });
    const r = await trocarSenha(novaSenha);
    if (r.ok) {
      setNovaSenha("");
      setConfirmaSenha("");
      setStatusSenha({ tipo: "ok", msg: "Senha alterada" });
      setTimeout(() => setStatusSenha({ tipo: "idle" }), 2500);
  ...
  const cnsBadge = () => {
    ...
    return validarCns(perfil.cns) ? (
      <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-700 ring-1 ring-emerald-200">
        <ShieldCheck className="size-3" /> válido
      </span>
    ) : (
      <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-rose-50 px-2 py-0.5 text-[10px] font-medium text-rose-700 ring-1 ring-rose-200">
  ...
  const mesInput =
    perfil.competenciaPadrao.length === 6
      ? `${perfil.competenciaPadrao.slice(0, 4)}-${perfil.competenciaPadrao.slice(4, 6)}`
      : "";
        ...
        {carregando ? (
          ...
          <div className="space-y-5">
            ...
            <Card
              ...
              desc="Usados na confirmação eletrônica do Responsável."
            >
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  <Field
                    icon={<IdCard className="size-3.5 text-muted-foreground" />}
                  ...
                  >
                    <input
                      ...
                      maxLength={15}
                      value={perfil.cns}
                      onChange={(e) => set("cns", e.target.value.replace(/\D/g, "").slice(0, 15))}
                      placeholder="15 dígitos"
                    />
                  </Field>
                </div>
                <Field
                  icon={<Stethoscope className="size-3.5 text-muted-foreground" />}
                  label="CBO (opcional)"
                >
                  <input
                    className={inputCls}
                    ...
                    maxLength={6}
                    value={perfil.cbo}
                    onChange={(e) => set("cbo", e.target.value.replace(/\D/g, "").slice(0, 6))}
            ...
            <Card
              ...
              desc={
                configDaOrg
                  ? "Cabeçalho do arquivo magnético (BPA). Definido pela organização."
                  : "Cabeçalho do arquivo magnético (BPA). Sincronizado na sua conta."
              }
            >
              {configDaOrg && (
                <p className="mb-3 rounded-lg border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
                  Estes dados agora são da <strong>organização</strong> (valem para todos os
                  usuários). Quem tem “gerenciar vínculos” edita em{" "}
                  <Link to="/admin" className="font-medium text-primary hover:underline">
                    Administração › Prefeitura e gestão
                  </Link>
                  .
                </p>
              )}
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  <Field label="Órgão de origem (nome)">
                    <input
                      className={inputCls}
                      maxLength={30}
                      disabled={configDaOrg}
                      value={perfil.config.orgaoOrigemNome}
                      onChange={(e) => setCfg("orgaoOrigemNome", e.target.value)}
                      placeholder="Ex.: SEC MUN SAUDE DE ..."
                    />
                  </Field>
                </div>
                <Field label="Sigla">
                  <input
                    ...
                    maxLength={6}
                    disabled={configDaOrg}
                    value={perfil.config.sigla}
                    onChange={(e) => setCfg("sigla", e.target.value)}
                    placeholder="SMS"
                  />
                </Field>
                <Field label="CNPJ/CPF (só números)">
                  <input
                    ...
                    maxLength={14}
                    disabled={configDaOrg}
                    value={perfil.config.cgcCpf}
                    onChange={(e) => setCfg("cgcCpf", e.target.value.replace(/\D/g, ""))}
                ...
                <div className="sm:col-span-2">
                  <Field label="Órgão de destino (nome)">
                    <input
                      ...
                      maxLength={40}
                      disabled={configDaOrg}
                      value={perfil.config.orgaoDestinoNome}
                      onChange={(e) => setCfg("orgaoDestinoNome", e.target.value)}
                      placeholder="Ex.: SES / SMS destino"
                    />
                  </Field>
                </div>
                <Field label="Tipo do órgão de destino">
                  <select
                    className={inputCls}
                    disabled={configDaOrg}
                    value={perfil.config.destinoTipo}
                    onChange={(e) => setCfg("destinoTipo", e.target.value as "M" | "E")}
                  >
            ...
            <Card
              ...
              title="Preferências"
              desc="Padrões do app e informações da conta."
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <Field
                  icon={<CalendarDays className="size-3.5 text-muted-foreground" />}
                  label="Competência padrão (opcional)"
                >
                  <input
                    type="month"
                    className={inputCls}
              ...
              {conta.criadoEm && (
                <p className="mt-3 text-xs text-muted-foreground">
                  Conta criada em {new Date(conta.criadoEm).toLocaleDateString("pt-BR")} · ID{" "}
                  <span className="font-mono">{conta.id?.slice(0, 8)}…</span>
                </p>
              )}
            </Card>
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                onClick={salvar}
                disabled={statusPerfil.tipo === "loading"}
            ...
            <Card
              ...
              title="Segurança"
              desc="Alterar a senha de acesso."
            >
              <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="mt-4 flex items-center justify-end gap-3">
                ...
                <button
                  onClick={salvarSenha}
                  disabled={statusSenha.tipo === "loading" || !novaSenha}
                ...
                >
                  {statusSenha.tipo === "loading" ? (
                    ...
                    <>
                      <Lock className="size-4" /> Trocar senha
                    </>
                  )}
                </button>
              </div>
